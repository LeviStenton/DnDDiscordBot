...
class SkillCheckEncounter(IEncounter):
    def __init__(self, user):
        self.tableName = "skillchecks"
        self.GenerateEncounter()
        super().__init__(
            title=self.encounter.name,
            description="Roll to test your skill!",
            colour=discord.Colour.red()
        )
        self.set_thumbnail(url=f"{self.encounter.picturePath}")
        self.set_author(name=f'Skill Check Encounter!', icon_url="https://i.pinimg.com/originals/48/cb/53/48cb5349f515f6e59edc2a4de294f439.png")
        ...
        self.set_footer(text=user.display_name + " - Rarity: " + str(self.encounter.rarity), icon_url=user.display_avatar)
